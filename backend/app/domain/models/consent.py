...
class Consent(Base):
    """
    Consent model for LGPD consent tracking.
    ...
    versao = Column(
        Integer, nullable=False, default=1, comment="Consent version number (increments on updates)"
    )
    ...
    titular_email = Column(String(255), nullable=True, comment="Data subject email (if applicable)")
    titular_documento = Column(
        String(100), nullable=True, comment="Data subject document (CPF, CNPJ) - encrypted"
    )

    # Consent details
    finalidade = Column(
        Text, nullable=False, comment="Specific purpose for data processing (LGPD Art. 9º)"
    )
    categorias_dados = Column(
        JSON, nullable=False, default=list, comment="Array of data categories covered by consent"
    )
    ...
    data_consentimento = Column(DateTime, nullable=True, comment="Timestamp when consent was given")

    # Revocation tracking
    revogado = Column(
        Boolean, nullable=False, default=False, index=True, comment="Whether consent was revoked"
    )
    data_revogacao = Column(DateTime, nullable=True, comment="Timestamp when consent was revoked")
    motivo_revogacao = Column(Text, nullable=True, comment="Reason for revocation (optional)")
    ...
    ip_origem = Column(String(45), nullable=True, comment="IP address where consent was collected")
    ...
    consentimento_marketing = Column(
        Boolean, nullable=False, default=False, comment="Consent for marketing communications"
    )
    ...
    consentimento_analise = Column(
        Boolean, nullable=False, default=False, comment="Consent for data analysis and profiling"
    )
    ...
    historico_alteracoes = Column(
        JSON, nullable=False, default=list, comment="Immutable array of all consent changes"
    )

    # User and tenant context
    coletado_por = Column(
        UUID(as_uuid=True), nullable=False, comment="User UUID who collected the consent"
    )
    tenant_id = Column(
        String(50), nullable=False, default="nacional", index=True, comment="Tenant identifier"
    )
    ...
    data_expiracao = Column(
        DateTime, nullable=True, comment="Consent expiration date (if applicable)"
    )
    ...
    def to_dict(self):
        ...
        return {
            ...
            "data_consentimento": (
                self.data_consentimento.isoformat() if self.data_consentimento else None
            ),
    ...
    def revogar(self, usuario_id: str, motivo: str = ""):
        ...
        """
        self.revogado = True
        self.data_revogacao = datetime.now(UTC)
    ...
    def is_valido(self) -> bool:
        ...
        if self.data_expiracao and datetime.now(UTC) > self.data_expiracao:
            return False

...
from app.domain.models.ingestion import IngestionMethod, IngestionSource
...
class IngestionCreateRequest(BaseModel):
    ...
    consentimento_id: Optional[UUID] = Field(
        None, title="Consent Id", description="LGPD consent UUID"
    )
    descricao: Optional[str] = Field(
        None, max_length=5000, title="Description", description="Description"
    )
    tags: Optional[List[str]] = Field(None, title="Tags", description="Tags array")
    metadata_adicional: Optional[Dict[str, Any]] = Field(
        None, title="Additional Metadata", description="Additional metadata"
    )
...
class IngestionCreateResponse(BaseModel):
    ...
    arquivo_storage_path: Optional[str] = Field(
        None, title="Storage Path", description="MinIO storage path"
    )
    confiabilidade_score: int = Field(
        ..., title="Reliability Score", description="Reliability score 0-100"
    )
...
class IngestionDetailResponse(BaseModel):
    ...
    dados_brutos_sample: Optional[List[Dict[str, Any]]] = Field(
        None, title="Raw Data Sample", description="Sample of raw data"
    )
...
class LineageNode(BaseModel):
    ...
    type: str = Field(
        ..., title="Type", description="Node type (ingestion, transformation, dataset)"
    )
    label: str = Field(..., title="Label", description="Node label")
    properties: Dict[str, Any] = Field(
        default_factory=dict, title="Properties", description="Node properties"
    )
...
class LineageEdge(BaseModel):
    ...
    properties: Dict[str, Any] = Field(
        default_factory=dict, title="Properties", description="Edge properties"
    )
...
class LineageResponse(BaseModel):
    ...
    dados_brutos_sample: Optional[List[Dict[str, Any]]] = Field(
        None, title="Raw Data Sample", description="First 100 rows"
    )
    transformacoes: List[str] = Field(
        ..., title="Transformations", description="Transformation steps"
    )
    confiabilidade_score: int = Field(
        ..., title="Reliability Score", description="Reliability score"
    )
    data_ingestao: datetime = Field(..., title="Ingestion Time", description="Ingestion timestamp")
...
class ConsentRecord(BaseModel):
    ...
    data_consentimento: Optional[datetime] = Field(
        None, title="Consent Date", description="Grant date"
    )
    data_revogacao: Optional[datetime] = Field(
        None, title="Revocation Date", description="Revocation date"
    )
...
class LGPDReportResponse(BaseModel):
    ...
    pii_types_detected: Dict[str, int] = Field(
        ..., title="PII Types Detected", description="PII counts by type"
    )
    pii_details: List[PIITypeStat] = Field(
        default_factory=list, title="PII Details", description="Detailed PII statistics"
    )
    total_pii_instances: int = Field(
        ..., title="Total PII Instances", description="Total number of PII instances"
    )
    masking_actions: List[str] = Field(..., title="Masking Actions", description="Actions taken")
    consent_status: str = Field(
        ..., title="Consent Status", description="Consent validation status"
    )
    consent_records: List[ConsentRecord] = Field(
        default_factory=list, title="Consent Records", description="Consent details"
    )
    consentimento_id: Optional[UUID] = Field(None, title="Consent Id", description="Consent UUID")
    compliance_score: int = Field(
        ..., title="Compliance Score", description="Compliance score 0-100"
    )
    risk_level: str = Field(
        ..., title="Risk Level", description="Risk level: LOW, MEDIUM, HIGH, CRITICAL"
    )
    recommendations: List[str] = Field(
        ..., title="Recommendations", description="Compliance recommendations"
    )
    lgpd_articles_applicable: List[str] = Field(
        default_factory=list,
        title="LGPD Articles Applicable",
        description="Applicable LGPD articles",
    )
    data_analysis: str = Field(..., title="Data Analysis", description="Detailed analysis text")
    data_processamento: Optional[datetime] = Field(
        None, title="Processing Time", description="Processing timestamp"
    )

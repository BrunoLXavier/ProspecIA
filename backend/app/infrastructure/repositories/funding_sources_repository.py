...
class FundingSourcesRepository:
    ...
    def __init__(self, session: AsyncSession, kafka_producer: Optional[KafkaProducer] = None) -> None:
        """
    ...
    async def find_by_id(
        ...
        result = await self.session.execute(
            query,
            {"id": str(funding_source_id), "tenant_id": str(tenant_id)}
        )
    ...
    async def update(
        ...
        allowed_fields = {
            "name", "description", "type", "sectors", "amount",
            "trl_min", "trl_max", "deadline", "url", "requirements", "status"
        }
